            ...
            "columnDefs":[
                {
                    "render":function(data,type,row,meta){
                        ...
                        if(row.hNum>1 && row.diffNum>1){
                            return "<div class='text-left liveWrapper'><span style='width:55%;display:inline-block'><a class='clickLive'hFlag='0' liveId='"+row.liveid+"'><i class='iconfont tableIcon' title='展开'>&#xe600;</i></a>"+row.title+"</span>" +
                                "<span style='width:45%;display:inline-block'>"+starttime+"</span></div>"
                        }else{
                            return "<div class='text-left liveWrapper'><span style='width:55%;display:inline-block'><i></i>"+row.title+"</span><span style='width:45%;display:inline-block'>"+starttime+"</span></div>"
                        }
    ...
    function setUserTable(hospital){
        $("#userTableLive").DataTable({
            ...
            "columnDefs":[
                {
                    "render":function (data,type,row,meta) {
                        ...
                        if(row.hNum>1){
                            return "<div class='text-left liveWrapper1'><span style='width:55%;display:inline-block'><a class='clickLive' hFlag='0' liveId='"+row.liveid+"'><i class='iconfont tableIcon' title='展开'>&#xe600;</i></a>"+row.title+"</span>" +
                                "<span style='width:45%;display:inline-block'>"+starttime+"</span></div>"
                        }else{
                            return "<div class='liveWrapper1'><span style='width:55%;display:inline-block'><i></i>"+row.title+"</span><span style='width:45%;display:inline-block'>"+starttime+"</span></div>"
                        }
                    },
                    "targets":0
                },
            ...
            "drawCallback":function (setting) {
                ...
                var flag1=false;
                //一旦含有一个tableIcon，就将flag0改变成true
                $("#tblLiveApproval .liveWrapper1").each(function(index,item) {
                    if( $(item).find("i").hasClass("tableIcon")){
                        flag1  = true
                    }
                });
                if(flag1){
                    $("#tblLiveApproval .liveWrapper1").find("i").not(".tableIcon").addClass("blockArea")
                }
                //点击直播
                $(".clickLive").off().on("click",function(){
